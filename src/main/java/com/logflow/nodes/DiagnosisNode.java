...
public class DiagnosisNode extends AbstractWorkflowNode {
    ...
    protected NodeExecutionResult doExecute(WorkflowContext context) throws WorkflowException {
        ...
        // 获取输入数据（支持多输入）
        Object inputData = getDiagnosisInputData(context);
        if (inputData == null) {
     ...
     * 获取诊断输入数据（多输入配置）
     */
    private Object getDiagnosisInputData(WorkflowContext context) throws WorkflowException {
        InputDataProcessor.InputDataResult inputResult = processInputData(context);
        if (!inputResult.isSuccess()) {
            throw new WorkflowException(id, "诊断节点多输入处理失败: " + inputResult.getErrorMessage());
        }

        Object inputData = inputResult.getData();
        logger.info("诊断节点使用多输入模式, 输入模式: {}", inputResult.getMetadata().get("inputMode"));
        return inputData;
    }
    ...
    public static class DiagnosisResult {
        ...
        public List<DiagnosisIssue> getIssues() {
