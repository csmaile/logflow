...
public class OutputMigrationDemo {
    ...
    private static void demonstrateConsoleOutput(WorkflowEngine engine) {
        ...
        // 原有 OutputNode 方式 (已废弃)
        System.out.println("   📤 原有 OutputNode (已废弃):");
        Workflow oldWorkflow = WorkflowBuilder.create("old-console", "旧版控制台输出")
                .addInputNode("input", "输入")
                .addOutputNode("output", "输出", Map.of( // 废弃方法
                        "outputType", "console",
                        "format", "json"))
                .connect("input", "output")
                .build();

        WorkflowExecutionResult oldResult = engine.execute(oldWorkflow, Map.of("input", testData));
        System.out.printf("      结果: %s (%dms)\n",
                oldResult.isSuccess() ? "✅ 成功" : "❌ 失败",
                oldResult.getExecutionDurationMs());
    ...
    private static void demonstrateFileOutput(WorkflowEngine engine) {
        ...
        // 原有 OutputNode 方式 (已废弃)
        System.out.println("   📤 原有 OutputNode 文件输出 (已废弃):");
        Workflow oldFileWorkflow = WorkflowBuilder.create("old-file", "旧版文件输出")
                .addInputNode("input", "输入")
                .addOutputNode("output", "文件输出", Map.of( // 废弃方法
                        "outputType", "file",
                        "filePath", testFile,
                        "append", true))
                .connect("input", "output")
        ...
        WorkflowExecutionResult oldFileResult = engine.execute(oldFileWorkflow, Map.of("input", testData));
        System.out.printf("      结果: %s\n",
                oldFileResult.isSuccess() ? "✅ 成功" : "❌ 失败");
    ...
    private static void demonstrateContextOutput(WorkflowEngine engine) {
        ...
        // 原有 OutputNode 方式 (已废弃)
        System.out.println("   📤 原有 OutputNode 上下文输出 (已废弃):");
        Workflow oldContextWorkflow = WorkflowBuilder.create("old-context", "旧版上下文输出")
                .addInputNode("input", "输入")
                .addOutputNode("output", "上下文输出", Map.of( // 废弃方法
                        "outputType", "context",
                        "contextKey", "old_result"))
                .connect("input", "output")
                .build();

        WorkflowExecutionResult oldContextResult = engine.execute(oldContextWorkflow, Map.of("input", testData));
        System.out.printf("      结果: %s\n",
                oldContextResult.isSuccess() ? "✅ 成功" : "❌ 失败");
        ...
        WorkflowExecutionResult newContextResult = engine.execute(newContextWorkflow, Map.of("demo_data", testData));
        System.out.printf("      结果: %s\n",
    ...
    private static void demonstrateEnhancedFeatures(WorkflowEngine engine) {
        ...
        WorkflowExecutionResult detailedResult = engine.execute(detailedWorkflow, Map.of("demo_data", testData));
        System.out.printf("      结果: %s\n",
        ...
        WorkflowExecutionResult combinedResult = engine.execute(combinedWorkflow, Map.of("demo_data", testData));
        System.out.printf("      结果: %s\n",
