...
public class LogAnalysisExample {
    ...
    private static void runBasicErrorDetection(WorkflowEngine engine) {
        ...
        Workflow workflow = new WorkflowBuilder("basic_error_detection", "基础错误检测")
            ...
            // 输出节点：控制台输出
            .addOutputNode("console_output", "控制台输出", 
                WorkflowBuilder.config(
    ...
    private static void runPerformanceAnalysis(WorkflowEngine engine) {
        ...
        Workflow workflow = new WorkflowBuilder("performance_analysis", "性能分析")
            // 数据源节点：生成模拟性能日志
            .addDataSourceNode("perf_data_source", "性能日志数据源", 
                Map.of(
                    "sourceType", "mock",
            ...
            // 输出节点：文件输出
            .addOutputNode("file_output", "文件输出", 
                WorkflowBuilder.config(
    ...
    private static void runComplexDiagnosis(WorkflowEngine engine) {
        ...
        Workflow workflow = new WorkflowBuilder("complex_diagnosis", "复合诊断工作流")
            ...
            // 数据源节点：模拟从文件读取日志
            .addDataSourceNode("log_file_reader", "日志文件读取器", 
                Map.of(
                    "sourceType", "mock",
                    "mockType", "mixed_logs",
                    "count", 200,
            ...
            // 输出节点：综合报告
            .addOutputNode("report_output", "报告输出", 
                WorkflowBuilder.config(
            ...
            // 输出节点：控制台摘要
            .addOutputNode("summary_output", "摘要输出", 
                WorkflowBuilder.config(
                    "inputKey", "final_report",
                    "outputType", "console"
                ))
        ...
        if (result.isSuccess()) {
            ...
            System.out.println("错误信息: " + result.getMessage());
            // 输出失败节点的详细信息
            result.getFailedNodeResults().forEach((nodeId, nodeResult) -> 
                System.out.println("节点 " + nodeId + " 失败: " + nodeResult.getMessage()));
        }
        System.out.println();
    }
}
